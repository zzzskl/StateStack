...
describe('closureService', () => {
    ...
    it('push 后 getChain 应返回正确顺序的 chain', () => {
        const closure = initClosure();
        const fnA = (prev) => (x) => prev(x) + 1;
        const fnB = (prev) => (x) => prev(x) * 2;

        closure.push({ pop: fnA });
        closure.push({ pop: fnB });
    ...
    it('不同函数名应互不干扰', () => {
        const closure = initClosure();
        closure.push({ pop: (prev) => prev, push: (prev) => prev });
