...
```
createStateStack(definition: StateStackDefinitionObject, runParent?: () => void): StateStackInstance
```
...
| `peek` | `(simplePeek: () => any) => any` | | 自定义 peek 逻辑，包装基础 peek |
| `push` | `(simplePush: (data) => void, data: any) => void` | | 自定义 push 逻辑 |
| `pop` | `(simplePop: () => any, simpleWriteResultData: (value) => void) => void` | | 自定义 pop 逻辑，可在此写入最终结果 |
| `statusDispatcher` | `(peek: () => any, status: string \| null) => string \| null` | ✓ | 返回当前要执行的状态名；返回 `null` 终止状态机 |
| `init` | `(state: StateSnapshot, push: (data) => void) => void` | | 初始化函数，`createStateStack()` 调用时立即执行 |
| `[statusName]` | `(state: StateSnapshot, peek: () => any, api: StateAPI) => void` | | 状态处理函数，以字段名作为状态名称 |
...
当 handler 调用的 `switchStatus` 携带 `{ effect: 'run' }` 且没有指定子栈时，控制权回传给 `runParent`。
...
返回的 `CurriedCreateStateStack` 可以继续传入 `refinementObject`（继续叠加）或 `definitionObject`（终止链，创建实例）。
...
interface StateStackRefinementObject {
    pop?:            (prevPop: () => any)               => () => any;
    push?:           (prevPush: (data: any) => void)    => (data: any) => void;
    peek?:           (prevPeek: () => any)              => () => any;
    switchStatus?:   (prevSwitchStatus: (...args) => void) => (...args) => void;
    writeResultData?:(prevWriteResultData: (value) => void) => (value) => void;
    writeExtra?:    (prevWriteExtra: (value) => void)   => (value) => void;
}
```
...
| 方法 | 说明 |
|------|------|
| `run()` | 启动/运行状态机。`init` 在实例创建时已执行；`run()` 从 `statusDispatcher` 开始，循环执行 `statusDispatcher → handler → effect` 直到状态为 `null`。幂等——重复调用在状态结束后无效果 |
...
## `StateAPI` — handler 第三个参数

状态处理函数的 `api` 参数包含以下方法：
...
| 方法 | 限制 | 说明 |
|------|------|------|
| `switchStatus(nextStatus, effect?)` | **每轮 ≤1** | 切换状态 + 携带 effect 描述。`nextStatus = null` 表示结束 |
...
| `{ effect: 'run' }` | 控制权回传给 `runParent` 回调 |
| `{ effect: 'run', param: ['child', id] }` | 运行指定子栈，子栈结束后回到当前栈继续 |

注意：
- `param` 为 `push` 提供入栈数据，格式为 `[data]`（数组包裹）
...
| `createStateStack` 参数非法（非对象/null/数字等） | `createStateStack: 参数必须是 StateStackDefinitionObject（含 state 字段）或 StateStackRefinementObject（不含 state 字段）` |
| 子栈 ID 不存在 | `childStateStack id "xxx" not found` |
| `createChildStateStack` 未在 runningScope 中实现（早期版本） | `createChildStateStack not implemented in this iteration` |
